...
StatelessWidget 和 StatefulWidget 是 组合类 的控件，因此他不负责绘制，本质上是通过 RenderObject 实现布局。在 RenderObject 中有个 createRenderObject() 方法返回 RenderObject 对象，RenderObject 实际负责实际的 layout() 和 paint()。

<https://docs.flutter.cn/get-started/fundamentals/widgets>


与 Flutter 中 Widget、Element、 RenderObject 三棵树的概念保持一致。 Widget 只保存组件配置信息，框架层内置了一个 createElement() 可以创建与之对应的 Element 实例。
...
* 特点：
  ...
  - 通过其自身的 `build` 方法构建UI界面。
  - 生命周期：build --- 唯一阶段。

* StatelessWidget 的 build 方法通常只在三种情况下调用：
  - StatelessWidget 第一次插入树中时；
  - 当 StatelessWidget 的父级改变其配置时；
...
StatefulWidget 用于构建动态交互UI界面，能够管理变化的内部状态，当状态改变时，组件会更新显示内容。例如：计数器、表单输入框。

StatefulWidget 实例 本身是不可变的，其可变状态存储在由 `createState` 方法创建的 单独的 `State` 对象中，或者 存储在该 State 订阅的对象中（如： Stream、ChangeNotificationer），这些对象的引用存储在 StatefulWidget 本身的 final 字段中。

* 特点：
  - StatefulWidget 关联2个类：Widget本身 和 单独的 State 类。
  - StatefulWidget 没有 build 方法，它的UI界面是通过关联其 State 对象来构建的。
  - 生命周期：createState、setState、dispose
...
InheritedWidget 用于高效向下传递信息的组件基类。专门用于在 Widget树中自顶向下高效地共享数据，顶层组件提供数据，子孙节点直接获取。


# 五、重点 Widget
<https://docs.flutter.cn/get-started/fundamentals/widgets#important-widgets-to-know>
