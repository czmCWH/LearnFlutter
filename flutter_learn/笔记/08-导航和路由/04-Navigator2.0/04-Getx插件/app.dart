  ...
  Widget build(BuildContext context) {
    ...
    return GetMaterialApp(
      ...
      routingCallback: (Routing? value) {   // 拦截路由
        ...
        const tabRoute = [Routes.A, Routes.B, Routes.C, Routes.D];
        if ([...tabRoute, ].contains(value.current)) {  
          WidgetsBinding.instance.addPostFrameCallback((_) {
            Get.offAllNamed(Routes.root);   // 回到根路由
            int index = tabRoute.indexOf(value.current);
            if (Get.isRegistered<MainController>()) {
              Get.find<MainController>().changeTabIndex(index);    // 切换 tabbar
            }
          });
